...
def main():
    ...
    for row in csv.DictReader(open(KF + "/材料词频表_v1.1.csv", encoding="utf-8-sig")):
        ...
        if (row["口径"] or "").strip() == "超纲/变形":
            chaogang.add(w)
    cefr_b12 = {w for w, lv in cefr.items() if lv in ("B1", "B2") and ok(w) and sane(w) and w in freq_hukou}
    ...
    for row in csv.DictReader(open(PROJ + "/data/词汇总表_教学工作区.csv", encoding="utf-8-sig")):
        mat_cov[row["词"].strip().lower()] = int(row["材料类覆盖数"])
    ...
    with open(OUT + "/灰区_v3.tsv", "w", encoding="utf-8") as f:
        f.write("词\t原因\n")
